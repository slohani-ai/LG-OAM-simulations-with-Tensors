...
class LG_Lights_Tensorflow:
    ...
    def LG_State(self,p_l_w):
        ...
        phase_due_l = tf.math.exp(-tf.cast(1j,tf.complex128)*l*tf.cast(np.angle(self.xx+1j*self.yy), tf.complex128))
        # a_r = np.sqrt(2 / np.pi) * (1 / w) * np.exp(-self.r ** 2 / w ** 2)
        # R = np.sqrt(1 / factorial(np.abs(l))) * (self.r * np.sqrt(2) / w) ** np.abs(l)
        # phase_due_l = np.exp(-1j*l*(np.angle(self.xx+1j*self.yy)))
        def p_0():
            if self.verbose:
                print ('|Found P = 0, ... ...')
    ...
    def Superposition(self,p_l_array,alpha_array,w=0,grating_period = 0,save_image=False,rank=0):
        device = ['GPU' if tf.config.list_physical_devices('GPU') else 'CPU']
        if self.verbose:
            if device[0] == 'GPU':
        ...
        with tf.device(device[0]):
            ...
            tf.debugging.assert_equal(p_l_array.shape[1],2,\
                            f'|p_l_array should be in [?,2] shape, but found in {p_l_array.shape}')
            tf.debugging.assert_equal(len(alpha_array),p_l_array.shape[0],\
            r'|alpha_array should match with len of $\ell$')
            w_mat = tf.reshape(tf.repeat(w,len(p_l_array)),[-1,1])
            p_l_array = tf.cast(p_l_array,tf.float32)
            p_l_w_array = tf.concat([p_l_array,w_mat],axis=1)
            p_l_w_array = tf.cast(p_l_w_array,tf.complex128)
    ...
    def Superposition_Batch(self,p_l_array,alpha_array,w=0,grating_period = 0,save_image=False):
        tf.debugging.assert_equal(p_l_array.ndim,3,'p_l_array should be 3 dim')
        tf.debugging.assert_equal(alpha_array.ndim, 3, 'alpha_array should be 3 dim')
